  ...
  <div class="container page-container">
    <PageTitle :title="data.title">
      ...
      <Link
        ...
        :href="route('customer.bidRequests.create')"
      ...
      <div class="col-md-12">
        ...
        <div class="data-container">
          <Table
            ...
            :tableStyle="'alert table-striped'"         
          >
            <template v-slot>
              <tr class="mb-2" v-for="(task, index) in data.tasks" :key="index">
                <td class="col-md-6">
                  <Link :href="route('customer.tasks.show', task.uuid)">{{ task.number }}</Link>                  
                  <div><small class="text-muted">{{ task.service.name }}</small></div>
...
export default {
  ...
  methods: {
    getStatistics() {
      let scope = this;
      axios
        .post(route("customer.dashboard.statistics"))

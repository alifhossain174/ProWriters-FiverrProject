        ...
        <div class="data-container">
          ...
          >
            <template v-slot>
              ...
              >
                <td class="col-md-5">
                  <Link :href="route('freelancer.tasks.show', task.uuid)">{{ task.number }}</Link>                 
                  <div class="taskcard__workType">{{ task.service.name }}</div>
                  <span class="badge text-bg-dark" v-if="task.is_archived_for_freelancer">{{ __('Archived') }}</span>
                </td>
                <td class="col-md-2">
                  {{ localDateTime(task.dead_line_for_freelancer) }}
                </td>
                <td class="col-md-2">
                  <span

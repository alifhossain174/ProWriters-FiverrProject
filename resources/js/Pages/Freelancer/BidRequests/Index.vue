      ...
      <div class="col-md-12" v-if="data.applied_bids.length > 0">
        ...
        <legend class="float-none w-auto px-3 fs-8">
          ...
          <Table
            ...
            :tableStyle="'alert alert-primary table-striped'"
          >
            <template v-slot>
              <tr
                class="mb-2"
                v-for="(bid, index) in data.applied_bids"
                :key="index"
              >
                <td class="col-md-3">
                  <Link
                    :href="
                      route('freelancer.bidRequests.show', bid.bid_request.uuid)
                    "
                    >{{ bid.bid_request.number }}</Link
                  >                  
                </td>
                <td class="col-md-2">
                ...
                <td class="col-md-2 text-end">
                  {{ formatMoney(bid.freelancer_payment_amount) }}
                </td>
              </tr>
            </template>
          </Table>
            ...
            <template v-slot>
              ...
              >
                <td class="col-md-3">
                  <Link
                    :href="
                      route('freelancer.bidRequests.show', bid_request.uuid)
                    "
                    >{{ bid_request.number }}</Link
                  >
                  <div class="taskcard__workType">

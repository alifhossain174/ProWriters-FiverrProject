      ...
      <div class="col-md-7">
        <fieldset class="border rounded-3 p-3" v-if="data.is_billable">
          <div>
            <form
              @submit.prevent="
                form.post(route('freelancer.paymentRequests.store'), formConfig)
              "
            >
              <Input
                v-model="form.name"
        ...
        >
          <template v-slot>
            <tr v-for="(task, index) in tasks" :key="index">
              ...
              <td class="col-md-5">
                <Link :href="task.link">{{ task.number }}</Link>
              </td>
...
export default {
  ...
  methods: {
    getAddress() {
      let profile = this.data.freelancer.profile;
      ...
      if (this.data.freelancer.country) {
        address = address + "\n" + this.data.freelancer.country.name;
      }
      return address;
    },
  },
  data() {
    return {
      form: this.$inertia.form({
        name: this.data.freelancer.full_name,

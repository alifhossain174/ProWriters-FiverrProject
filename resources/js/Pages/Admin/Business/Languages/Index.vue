<template>
  <BusinessLayout :title="data.title">
    <TitleBar :title="data.title" :create_link="route('admin.languages.create')"/>
    <SearchBar :url="route('admin.languages.index')" :filters="filters.filters" :hide_inactive_search="true" />
    ...
    <Table :options="tableOptions" :links="languages.links" :total="languages.total">
      <template v-slot>
        <tr v-for="(language, index) in languages.data" :key="index">
          <td>
            <Link :href="route('admin.languages.edit', language.id)">{{ language.name }}</Link>
          </td>   
          <td class="text-end">
            {{ (language.percentage) ? parseFloat(language.percentage) : 0}}%
          </td>              
          <td class="col-md-2 text-end">
            <DestroyButton :delete_url="route('admin.languages.destroy', language.id)"/>
          </td>
        </tr>
      </template>
    </Table>

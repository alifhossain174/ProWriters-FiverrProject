<template>
  <Modal :title="data.title">
    <form
      @submit.prevent="form.patch(route('admin.services.update', existing_record.id))"
    >
      ...
      <div v-if="existing_record.id != data.service_types.resume_writing">
        <div class="mt-4">
          <CheckBox
            v-model="form.disable_writing"
            name="disable_writing"
            :label="__('Disable Writing')"
          />
          <CheckBox
            v-model="form.disable_editing"
            name="disable_editing"
            :label="__('Disable Editing')"
          />
          <CheckBox
            v-model="form.disable_proofreading"
            name="disable_proofreading"
            :label="__('Disable Proofreading')"
          />
        </div>
      </div>

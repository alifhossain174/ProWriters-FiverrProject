    ...
    <Table :options="tableOptions" :links="additional_services.links" :total="additional_services.total">
      <template v-slot>
        <tr v-for="(additionalService, index) in additional_services.data" :key="index">
          <td>
            <Link :href="route('admin.additionalServices.edit', additionalService.id)">{{ additionalService.name }}</Link>
          </td>               
          <td>
          ...
          <td class="col-md-2 text-end">
            <DestroyButton :delete_url="route('admin.additionalServices.destroy', additionalService.id)"/>
          </td>
        </tr>
      </template>
    </Table>

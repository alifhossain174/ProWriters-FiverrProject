  ...
  <Modal :title="data.title">
    <form
      @submit.prevent="
        existing_record
          ? form.patch(
              route('admin.authorLevels.update', existing_record.id)
            )
    ...
    >
      ...
      <fieldset class="border rounded-3 p-3 mb-4">
        ...
        <div class="row fs-8">
          ...
          <div class="col-md-6">
            <Input
              v-model="form.author_price_per_word_writing"
              name="price"
              :label="__('Author Price')"
              :required="true"
              @keypress="
                onlyNumber($event, form.author_price_per_word_writing)
              "
            />
          </div>
        </div>
      ...
      <fieldset class="border rounded-3 p-3 mb-4">
        ...
        <div class="row fs-8">
          ...
          <div class="col-md-6">
            <Input
              v-model="form.author_price_per_word_editing"
              name="price"
              :label="__('Author Price')"
              :required="true"
              @keypress="
                onlyNumber($event, form.author_price_per_word_editing)
              "
            />
          </div>
        </div>
      ...
      <fieldset class="border rounded-3 p-3 mb-4">
        ...
        <div class="row fs-8">
          ...
          <div class="col-md-6">
            <Input
              v-model="form.author_price_per_word_proofreading"
              name="price"
              :label="__('Author Price')"
              :required="true"
              @keypress="
                onlyNumber($event, form.author_price_per_word_proofreading)
              "
            />
          </div>
        </div>
      ...
      <Input
        ...
        :label="
          __('Number of tasks the author can have in progress at a time')
        "
        :required="true"        
  ...
  methods: {
    prepareForm() {
      let inputs = {
        ...
        author_price_per_word_writing: 0,
        customer_price_per_word_editing: 0,
        author_price_per_word_editing: 0,
        customer_price_per_word_proofreading: 0,
        author_price_per_word_proofreading: 0,

<template>
  <BusinessLayout :title="data.title">
    ...
    <SearchBar :url="route('admin.authorLevels.index')" :filters="filters.filters" :hide_inactive_search="true" />
        
    <Table :options="tableOptions" :links="author_levels.links" :total="author_levels.total">
      <template v-slot>
        <tr v-for="(authorLevel, index) in author_levels.data" :key="index">
          <td>
            <Link :href="route('admin.authorLevels.edit', authorLevel.id)">{{ authorLevel.name }}</Link>
          </td>
          <td class="text-center">            
            <span v-if="authorLevel.is_default"><i class="fa-solid fa-circle-check text-success"></i></span>
          </td>                    
          <td class="text-center">
            {{ authorLevel.numeric_value  }}
          </td>  
          <td class="text-center">            
            <span v-if="authorLevel.is_popular"><i class="fa-solid fa-circle-check text-success"></i></span>
          </td>                   
          <td class="col-md-2 text-end">
            <DestroyButton :delete_url="route('admin.authorLevels.destroy', authorLevel.id)"/>
          </td>
        </tr>
      </template>
    </Table>

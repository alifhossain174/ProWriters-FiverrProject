<template>
  <SettingsLayout :title="data.title">
    ...
    <ul class="nav nav-tabs">
      <li
        v-for="(gateway, index) in data.gateways"
        :key="index"
        class="nav-item"
      >
        <Link
          :href="
            route('admin.settings.payment.gateways', { gateway: gateway.slug })
          "
          class="nav-link"
          :class="{ active: data.current_gateway == index }"
          >{{ __(gateway.name) }}</Link
        >
      </li>
    </ul>

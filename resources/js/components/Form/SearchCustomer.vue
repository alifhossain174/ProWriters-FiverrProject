<template>
    <div :class="bottom_margin ? bottom_margin : 'mb-3'" @click="$page.props.errors[name] = null">
        <label class="form-label">{{ label }} </label>
        <v-select v-model="model" :placeholder="__('Start typing to search for a customer')" :reduce="(option) => option.id"
            label="full_name" :filterable="false" :options="options" @search="onSearch">
            <template v-slot:no-options="{ search, searching }">
                <template v-if="searching">
                    {{ __("No results found for") }} <em>{{ search }}</em>.
                </template>
                <em v-else style="opacity: 0.5">{{ __("Start typing to search for a customer") }}.</em>
            </template>

...
class SubmitWorkController extends Controller
{
    ...
    public function index(Request $request, Task $task)
    {
        ...
        return inertia('Freelancer/Tasks/Works/Index', [
            'tab'   => 'works',
            'task'  => $task,
    ...
    public function create(TaskMessageService $taskMessageService, Task $task)
    {
        ...
        return Inertia::modal('Freelancer/Tasks/Works/Create', [

            'task' => $task,
            'data' => [
                'title'  => __('Submit Work'),
                'config' => $taskMessageService->getConfigForCreateMessage(route('attachments.store'), route('freelancer.tasks.works.store', $task->uuid)),

            ],
        ])->baseRoute('freelancer.tasks.works.index', $task->uuid);
    }
    ...
    public function store(Request $request, Task $task)
    {
        ...
        return redirect()->route('freelancer.tasks.works.index', $task->uuid)->withSuccess(__('Successfully submitted'));
    }
}

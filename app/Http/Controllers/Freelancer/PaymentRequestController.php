...
class PaymentRequestController extends Controller
{
    ...
    {
        return inertia('Freelancer/PaymentRequests/Index', [
            'data'             => [
            ...
            'balance'          => fn()          => Bill::where('freelancer_id', auth()->user()->id)->whereNull('paid')->sum('total'),
            'filters'          => $request->only('search'),
            'payment_requests' => Bill::where('freelancer_id', auth()->user()->id)                
    ...
    {
        $tasks             = $this->getBillableTasks();
        $user['full_name'] = auth()->user()->full_name;
        $user['profile']   = FreelancerProfile::select(['address', 'city', 'state'])->where('user_id', auth()->user()->id)->get()->first();
        $user['country']   = Country::where('code', auth()->user()->country_code)->get()->first();
        $total             = $this->getTotalBillableAmount($tasks);

        $payout_threshold = $this->getPayoutThresholdAmount();

        return inertia('Freelancer/PaymentRequests/Create', [
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'invoice_number' => 'sometimes|unique:bills,freelancer_id',
        ], [
        ...
        try {
            ...
            foreach ($tasks as $task) {
                ...
                $item = new BillItem([
                    'task_id' => $task->id,
                    'total'   => $task->freelancer_payment_amount,
                ]);
    ...
    public function show(Bill $paymentRequest)
    {
        ...
        return inertia('Freelancer/PaymentRequests/Show', [
            'data'           => [
                ...
                'allow'              => [
                    'archiving'   => ($paymentRequest->paid && !$paymentRequest->is_archived_for_freelancer) ? true : false,
                    'unarchiving' => ($paymentRequest->is_archived_for_freelancer) ? true : false,
                ],
            ],
    ...
    public function archive(Bill $paymentRequest)
    {
        ...
        $paymentRequest->is_archived_for_freelancer = true;
        $paymentRequest->save();
    ...
    {
        $paymentRequest->is_archived_for_freelancer = null;
        $paymentRequest->save();

        return redirect()->back()->withSuccess(__('Unarchived'));
    }

    private function getTotalBillableAmount($tasks)
    {
        $total = 0;
        if ($tasks->count() > 0) {
            $total = $tasks->sum('freelancer_payment_amount');
        }
        return $total;
    }

    private function getBillableTasks()
    {
        return Task::select('id', 'uuid', 'number', 'freelancer_payment_amount')
            ->whereNull('is_billed')

...
use App\Models\ProjectManagement\ResumeWriting;
...
class Task extends Model
{
    ...
    public function details()
    {
        if ($this->service_id == ServiceType::ACADEMIC_WRITING) {
            return $this->hasOne(AcademicWriting::class, 'task_id', 'id');
        }
        if ($this->service_id == ServiceType::CONTENT_WRITING) {
            return $this->hasOne(ContentWriting::class, 'task_id', 'id');
        }
        if ($this->service_id == ServiceType::RESUME_WRITING) {
            return $this->hasOne(ResumeWriting::class, 'task_id', 'id');
        }
    }
    ...
    {
        return $this->hasOne('App\Models\Business\Urgency', 'id', 'urgency_id');
    }
    ...
    public static function getNumberOfWordsAndPages($quantity, $unit_name, $spacing_type = null)
    {
        $pages      = 0;
        $words      = 0;
    ...
    public function scopeWithDetails($query, array $load = [])
    {
        $this->load(array_merge([
            'status', 'service', 'urgency', 'serviceLevel', 'authorLevel', 'additionalServices',
        ], $load));

        if ($this->service_id == ServiceType::ACADEMIC_WRITING) {
            $this->details                            = $this->details()->with(['assignment', 'subject', 'academicLevel', 'paperFormat'])->get()->first();
            $this->details->number_of_words_and_pages = $this->getNumberOfWordsAndPages($this->details->quantity, $this->details->unit_name, $this->details->spacing_type_id);
        } else if ($this->service_id == ServiceType::CONTENT_WRITING) {
            $this->details                            = $this->details()->with(['assignment', 'subject', 'language', 'grammaticalPerson'])->get()->first();
            $this->details->number_of_words_and_pages = $this->getNumberOfWordsAndPages($this->details->quantity, $this->details->unit_name);
        } else if ($this->service_id == ServiceType::RESUME_WRITING) {
            $this->details = $this->details()->with(['assignment'])->get()->first();
        }
        return $this;
    }
    ...
    public function scopeLoadLatestSubmittedWork()
    {
        $this->load([
            ...
            'submittedWorks.user'        => function ($q) {
                $q->select(['id', 'uuid', 'type', 'photo', 'code']);
            },
            'rating',
        ]);
